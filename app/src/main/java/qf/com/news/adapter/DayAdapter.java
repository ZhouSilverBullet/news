...
public class DayAdapter extends SuperAdapter<DayBean.ItemsBean, DayAdapter.ViewHolder> {

    private static MediaPlayer player;
    private static DayAdapter.ViewHolder playHolder;

    public DayAdapter(Context context, List<DayBean.ItemsBean> list, int[] layoutId) {
    ...
    public void onBindViewHolder(ViewHolder holder, final int position) {
        ...
        final DayBean.ItemsBean itemsBean = list.get(position);
        final String url = imgUrl1 + itemsBean.getId() / 10000 + "/" + itemsBean.getId() + "/medium/" + itemsBean.getImage();
        DayBean.ItemsBean.UserBean user = itemsBean.getUser();
        if (user == null) {
            return;
        }
    ...
    public int getToChild(int position) {
        DayBean.ItemsBean itemsBean = list.get(position);
    ...
    @Override
    public void onViewDetachedFromWindow(DayAdapter.ViewHolder holder) {
    ...
    static class ViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener, MediaPlayer.OnPreparedListener, MediaPlayer.OnCompletionListener {
        ...
        private DayBean.ItemsBean data;
