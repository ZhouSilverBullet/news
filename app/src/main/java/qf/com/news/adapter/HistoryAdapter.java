...
public class HistoryAdapter extends SuperAdapter<HistoryBean.ItemsBean, HistoryAdapter.ViewHolder> {

    private static MediaPlayer player;
    private static HistoryAdapter.ViewHolder playHolder;

    public HistoryAdapter(Context context, List<HistoryBean.ItemsBean> list, int[] layoutId) {
    ...
    public void onBindViewHolder(ViewHolder holder, final int position) {
        ...
        final HistoryBean.ItemsBean itemsBean = list.get(position);
        final String url = imgUrl1 + itemsBean.getId() / 10000 + "/" + itemsBean.getId() + "/medium/" + itemsBean.getImage();
        HistoryBean.ItemsBean.UserBean user = itemsBean.getUser();
        if (user == null) {
            return;
        }
    ...
    public int getToChild(int position) {
        HistoryBean.ItemsBean itemsBean = list.get(position);
    ...
    static class ViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener, MediaPlayer.OnPreparedListener, MediaPlayer.OnCompletionListener {
        ...
        private HistoryBean.ItemsBean data;
